...
| `sensitive` | Content that looks like it holds credentials/secrets | filenames flagging "取扱注意" / "confidential" / "credentials", connection-string sheets |
...
2. **Path/folder keyword heuristics** — folder and filename names are often the strongest
   signal, especially in non-English projects. Real examples seen in practice: `要件定義書`
   (requirements spec) and `ユーザーマニュアル` (user manual) → `document`; `ソースコード` /
   `_output_sourcecode` / `output/full` → `source-code`; `DB接続情報` (DB connection info) →
...
const SENSITIVE_HINTS = ['取扱注意', 'confidential', 'credential', 'password', '接続情報'];
