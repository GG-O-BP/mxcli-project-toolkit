...
**Example (correct):**
> Page `PayerDetail_NewEdit`, Section D (`ctnSec4` / 一般データ): textbox `txtAccountGroup` (AccountGroup) → Editable = Never
...
| 4 | `extraction/knowledge-base/share/KB_*.md` | Requirements detail, field labels, uSoner/SAP API specs |
...
1. **Collect:** run `./mxcli docker check -p Macnica-TestRunOS.mpr` to get full CE error list.
...
```bash
# Before starting
cp Macnica-TestRunOS.mpr Macnica-TestRunOS.mpr.backup

# After verifying success
rm Macnica-TestRunOS.mpr.backup

# If Studio Pro crashes or MPR is corrupt
cp Macnica-TestRunOS.mpr.backup Macnica-TestRunOS.mpr
```
...
**Do NOT use** `git checkout HEAD -- Macnica-TestRunOS.mpr` as recovery — it discards all good MPR changes since the last commit. Use the `.backup` file.
...
3. `Share/WorkFlow/設計書/07_Form.md` — section structure
