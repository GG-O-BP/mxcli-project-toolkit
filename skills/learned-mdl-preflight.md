...
| 9 | Set an association inline in a CHANGE or CREATE activity: `change $Obj (Module.AssocName = $Other)` or `create Entity (Module.AssocName = $Other)` | **STOP → 버전 확인.** mxcli nightly-5(2026-07-09)+Mendix 11.12.0 GA에서 CHANGE 인라인 설정 정상 검증(2026-07-13, 단일 시나리오 — BUG-21 status update 참조): mxbuild 게이트+SP 로드 확인 전제로 직접 경로 허용. 구버전 mxcli는 **STOP → MCP** (`ped_create_document`/`ped_update_document`). 참고: MCP 백엔드는 기존 MF 수정(UpdateMicroflow) 미지원 — 수정은 JSON 패치(ped_update_document)만 가능 | mxcli writes the association name as an `AttributeIdentifier` in the CHANGE/CREATE BSON — Studio Pro rejects the model on load. **Reading through an association (`$Obj/Module.Assoc/Target/Attr`) is safe; setting one inline in change/create is not.** |
