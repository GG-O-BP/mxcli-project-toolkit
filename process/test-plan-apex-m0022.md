# M-0022 Payer Registration — Test Plan (POC Phase)
...
| P6 | Studio Pro "Update All Widgets" on Payer_OrgChoice (org gallery CE0463) | Studio Pro | ⬜ |
...
|---|---|---|
| `demo_user` (or MxAdmin) | PayerRegistration.User | Yes — must have Employee with UserId = login name |

> **Risk:** `ACT_PayerDetail_InitNew` calls `Org_Master.GET_Employee_ByUserId($currentUser/Name)`. If no match exists, it shows a user error and returns empty — Payer_OrgChoice will not load correctly.

---

## 3. Happy path — new payer registration (no workflow)

### Scenario H1: Register a new payer (保存 / Save Draft)

| Step | Action | Expected result | Risk / Note |
|---|---|---|---|
| H1-1 | Navigate to PayerRegistration_Overview | Gallery loads; 0 records if fresh DB | — |
| H1-2 | Click ＋新規登録申請 | Payer_OrgChoice opens; ApplicantDept pre-filled from Employee record | Fails if no Employee (P3) |
| H1-3 | ContractorLocationCode shows "01" (default) | Field shows '01' | — |
| H1-4 | Click 法人検索 | SNP_CorporationSearchPopup opens as popup | Stub exists; popup layout |
| H1-5 | Enter any keyword → click 検索して選択 | Stub returns マクニカ株式会社; popup closes; SelectedCompanyName + SelectedCorporateNo populate on OrgChoice | STUB always returns hardcoded result regardless of keyword |
| H1-6 | Click 次へ | Payer_Confirm_Selection opens; shows company + org summary | P7f2 script needed |
| H1-7 | Click 支払人を作成 | PayerDetail_NewEdit opens | P7g script needed |
| H1-8 | Fill required fields (currency, etc.) | Form accepts input | Fields TBD in script |
| H1-9 | Click 保存 | ACT_PayerDetail_SaveDraft runs; PayerDetail created in DB; status → '01'; navigate to Overview | Wrapper MF needed (see §6) |
| H1-10 | Overview shows new record with status '01' | Row appears in gallery | Status shows raw '01' until CommonCategory seeded (P5) |
| H1-11 | Click 詳細 on the new record | PayerDetail_View opens | P7h script needed |

### Scenario H2: Submit for workflow approval (WF申請)
...
| H2-2 | Click WF申請 instead of 保存 | ACT_Payer_Submit runs: SaveDraft → validation → duplicate check → STUB_ACT_WFApplication_Submit |
| H2-3 | Status → '02' (申請中) on Overview | Record shows updated status |
| H2-4 | Admin approves via Workflow task page | Status → '03' (承認済) |

---

## 4. Key business logic to verify

### BL1 — Employee lookup in ACT_PayerDetail_InitNew
...
- **What:** ACT_PayerDetail_InitNew loops SalesOrganization records → creates ChoiceOrg_Dto NPEs linked to Dto
...
- **Pass:** SelectedCompanyName = 'マクニカ株式会社', SelectedCorporateNo = '1234567890123' after popup closes
- **Fail:** Fields remain empty; or popup doesn't close
- **Note:** STUB always returns マクニカ株式会社 regardless of keyword — expected behavior for POC

### BL4 — ACT_PayerDetail_SaveDraft creates all entities
- **What:** One save call creates: ApplicationCommonHeader + PayerApplicationHeader + PayerCustomerBase + PayerDetail + PayerAreaData
- **Pass:** Record appears in Overview; DB has all 5 entities linked correctly
- **Fail:** Partial create (one of the sub-calls fails); check Mendix log for error node 'PayerRegistration'
- **Risk:** Cross-module MF calls (BusinessApp_Common, Customer_Common) must succeed

### BL5 — ACT_Payer_Submit validation chain
- **What:** Submit runs: GET_PayerDetail_Dto → VAL_PayerDetail_BeforeSubmit → ACT_DuplicateCheck_Run → STUB_ACT_WFApplication_Submit
- **Pass:** Status changes to '02'; WF task created
- **Fail:** Silent failure (returns false + logs warning) — check Mendix log

### BL6 — Duplicate check
- **What:** ACT_DuplicateCheck_Run checks for existing payer with same company
...
| STUB search always returns マクニカ株式会社 | No real uSoner API connected | Phase 2 — replace STUB with real REST call |
...
| ChoiceOrg selection not persisted | ChoiceOrg_Dto NPE checked boxes are read in save microflow (if wired) | Review in ACT_PayerDetail_SaveDraft — ChoiceOrg records need commit |
...
|---|---|---|
| **ACT_PayerDetail_SaveDraft takes 2 params** | Signature: `(Dto, SearchResult)`. The save button on NewEdit can only pass objects in scope. DataView cannot use association (CE6705), so SearchResult is not in scope directly. | Wrapper MF: `ACT_PayerDetail_Save(Dto)` that navigates `$Dto/Assoc` to get SearchResult in the microflow body, then calls SaveDraft |
| **ChoiceOrg records not committed in SaveDraft** | ACT_PayerDetail_SaveDraft creates PayerDetail but does NOT loop ChoiceOrg_Dto list and create committed ChoiceOrg records | To be added in script 7g (NewEdit), or as an addition to ACT_PayerDetail_SaveDraft |
...
| 7h | 21-page-payerdetail-view.mdl | ⬜ | H1-11 |
