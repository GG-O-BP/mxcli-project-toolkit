...
- [ ] 기존 추출 파이프라인(`pipelines/outsystems/`, `pipelines/java-angular/`)이 이 소스 스택을 커버하는지, 혹은 수동 분석이 필요한지 아직 미정임을 인지했다
...
- [ ] 프롬프트 실행
  ...
  전체를 검토해줘. 기존 추출 파이프라인이 이 소스 스택을 커버하는지 판단하고,
...
- [ ] 프롬프트 실행
  ...
  ```
  skills/kb-generation.md 템플릿에 따라 sources/Non-SAP-source/의 문서·데이터 자료를
  분석해서 analysis/Non-SAP-source/knowledge-base/share/ 밑에 KB 마크다운을
  작성해줘. 필드 정의, 대표 샘플, 화면 구성요소를 포함해.
  ```
...
- [ ] 검증 프롬프트 실행
  ...
  ```
  skills/brd-validation.md 체크리스트로 방금 작성한 BRD JSON을 검증해줘.
...
- [ ] 프롬프트 실행
  ...
  ```
  skills/modularize-domain.md 기준에 따라 방금 검증된 BRD가 별도 Mendix 모듈이어야
  하는지, 기존 앱의 한 모듈로 흡수돼야 하는지 판단하고 근거 HTML 문서를 만들어줘.
...
- [ ] 프롬프트 실행
  ...
  fit-gap 분석을 작성해줘. 대상 프레임워크가 제공하지 않는 소스 의존 기능은
  fit-gap 항목으로 명시해줘.
  ```
...
- [ ] 프롬프트 실행
  ...
  ```
  skills/design-artifacts.md에 따라 sources/Non-SAP-source/의 참고 화면 자료를 바탕으로
...
- [ ] 프롬프트 실행
  ...
  ```
  skills/brd-to-build-plan.md에 따라 BRD + 3단계 아키텍처 산출물을 바탕으로
  번호 매겨진 의존성 순서 빌드 계획(엔티티→연관관계→마이크로플로우→페이지)을
  작성해줘.
  ```
...
- [ ] 프롬프트 실행
  ...
  ```
  skills/iterative-build-loop.md의 게이트에 따라 빌드 계획의 1번 스크립트부터
...
- [ ] 프롬프트 실행
  ...
  ```
  skills/e2e-harness-base.md에 따라 골든 패스(조회/등록/수정/삭제) Playwright 테스트를
  작성하고, 원본 데이터 샘플을 시드로 써줘. DB 검증은
  skills/learned-db-assertions.md대로 진행해줘.
  ```
