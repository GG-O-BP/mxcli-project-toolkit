...
모든 마이그레이션은 소스 스택과 무관하게 동일한 단계를 거칩니다. 각 단계마다 그 단계를 담당하는 스킬이 하나씩 있고, 각 스킬은 구체적인 산출물을 다음 단계로 넘깁니다. **이 흐름을 실제로 실행할 때 사용자가 단계별로 에이전트에 입력할 프롬프트 예시는 저장소 루트의 `CONVERSION-RUNBOOK.md`에 있습니다** (사용자용 실행 문서 — 스킬이 아닙니다):
...
```
mxcli-project-toolkit/
  CONVERSION-RUNBOOK.md         ← 사용자용 실행 문서(스킬 아님): 단계별로 에이전트에 입력할 프롬프트 예시 + 게이트 (P→0~6단계)
  skills/
