...
mxcli-project-toolkit/
  skills/
    conversion-runbook.md       ← 컨버전 전체 진행 런북: 순서대로 입력할 소스 무관 프롬프트 체크리스트 + 게이트 (P→0~6단계)
...
| 작업 | 로드할 스킬 |
|------|--------------|
| 컨버전 프로젝트를 처음부터 끝까지 진행 — 순서대로 입력할 소스 무관 프롬프트 체크리스트 (착수→트리아지→BRD→아키텍처→빌드→테스트) | `conversion-runbook.md` |
