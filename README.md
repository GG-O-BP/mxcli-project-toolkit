...
**새 컨버전을 설정할 때 이 규칙을 프로젝트 컨텍스트(`CLAUDE.local.md`)에 추가하세요** — 모든 에이전트 세션이 이 규칙에 구속되도록. 기존 프로젝트 컨텍스트의 "Screenshot & UX audit rule" 섹션을 템플릿으로 복사하면 됩니다.
...
| 도구 | 하는 일 | 사용 시점 |
|------|-------------|----------------|
| `assess-migration.md` | AI 주도 수동 인벤토리: 소스 파일을 읽고 엔티티, 비즈니스 로직, 연동, 보안, 마이그레이션 리스크를 다루는 사람이 읽기 좋은 마크다운 보고서를 생성합니다. | 항상 — 앱 크기와 무관하게 파이프라인 출력 위에 사람이 읽을 수 있는 레이어를 제공합니다(추출을 대체하지 않음). 추출 파이프라인 전이나 후에 실행하세요. |
| 추출 파이프라인 (`pipelines/java-angular/` · `pipelines/outsystems/`) | AST 기반 자동 추출: 소스 코드를 정규화된 KB JSON으로 파싱하고, BRD 매퍼를 실행하며, 모듈별 BRD와 HTML 보고서를 생성합니다. | 항상 — 앱 크기와 무관하게 세우고 실행합니다. 기존 파이프라인이 소스 스택을 커버하면 재사용하고, 아니면 새 extractor를 구축합니다. |

**Java/Spring 앱의 올바른 결합 흐름(앱 크기 무관):**
...
mxcli-project-toolkit/
  skills/
    ...
    source-triage.md            ← 추출 전 게이트: 커버리지 확인, 파이프라인 재사용 vs 신규 구축 판단, 범위 한정
...
| 작업 | 로드할 스킬 |
|------|--------------|
| 추출 파이프라인 선택(재사용 vs 신규 구축 — extractor는 앱 크기와 무관하게 항상 세움), 커버리지 확인, 대규모 소스의 범위 한정 | `source-triage.md` |
...
**프로젝트 산출물은 커밋하지 않습니다** — 컨버전은 이 클론 안에서 진행되지만 `analysis/`, `sources/`, `mendix/`, `knowledge-base/`, `*.mpr`, `CLAUDE.local.md`, `.claude/agents/`는 gitignore로 커밋에서 제외됩니다. 커밋되는 것은 재사용 자산(스킬, 버그 로그, 파이프라인 개선)뿐입니다.
