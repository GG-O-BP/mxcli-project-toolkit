...
핵심 설계 사상은 **"클론 하나가 곧 작업장"** 입니다. 컨버전은 이 저장소 클론 안에서 직접 진행됩니다 — 컨버전별 산출물은 `sources/<이름>/`(원본), `analysis/<이름>/`(분석·설계·빌드 계획·세션 노트), `mendix/<이름>/`(대상 .mpr), 루트 `CLAUDE.local.md`(프로젝트 컨텍스트)에 놓입니다. 이 영역은 전부 `.gitignore`로 커밋에서 제외되므로, **커밋되는 것은 재사용 자산뿐입니다** — `git pull` 한 번으로 최신 지식을 받으면서도 프로젝트 데이터는 로컬에만 남고, 재사용 가능한 패턴만 `skills/learned-*.md`로 승격됩니다.
...
```
Phase 1  샘플링       bun run.js 1    파일 구조 스캔 → schema.json (전체 추출 전 구조 검증)
Phase 2  추출 + 병합   bun run.js 2    추출기 병렬 실행 → extracted/*.json
                                       → merger.js: 중복 제거 + 교차 참조 링크
                                       → knowledge-base/*.json (entities, logics, screens …)
Phase 3  BRD 생성     bun run.js 3    모듈별로 5개 매퍼 실행 → {module}.brd.json
