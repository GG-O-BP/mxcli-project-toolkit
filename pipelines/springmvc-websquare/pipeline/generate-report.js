...
const moduleData = moduleNames.map(name => {
  ...
  return {
    ...
    screens: m.screens.map(s => ({
      ...
      hasConditionalStyling: s.widgetSummary?.hasConditionalStyling,
      gaps: s._gaps || []
    })),
...
<head>
<meta charset="UTF-8">
<title>SpringMVC/WebSquare Extraction Report</title>
<style>
...
<div class="sidebar">
  <div class="sidebar-header">
    <h1>SpringMVC/WebSquare Extraction Report</h1>
  ...
  <div class="page active" id="page-dashboard">
    ...
    <table style="margin-bottom:28px">
      ...
      <tbody>
        ...
        <tr><td>Server Logic (Controller/Service/DAO)</td><td>${logics.filter(l=>l.logicKind==='action'||l.logicKind==='process').length}</td><td>Microflow / Workflow</td></tr>
...
function showModule(name) {
      ...
      if (brd.useCases && brd.useCases.length) {
        ...
        brd.useCases.forEach(uc => {
          ...
          const oq   = Array.isArray(uc.openQuestions) && uc.openQuestions.length
                        ? uc.openQuestions.map(q => esc(typeof q === 'string' ? q : q.question)).join('<br>') : '—';
          const status = uc.reviewStatus === 'reviewed' ? 'reviewed' : (uc.status || 'code-inferred');
          const statusStyle = status === 'reviewed' || status === 'doc-confirmed'
  ...
  if (m.screens.length) {
    ...
    html += '<table><thead><tr><th>Name</th><th>List UI</th><th>Form UI</th><th>Conditional Styling</th><th>Gaps</th></tr></thead><tbody>';
    m.screens.forEach(s => {
      ...
      html += '<td>' + (s.hasConditionalStyling ? '<span class="gap-tag">review needed</span>' : '—') + '</td>';
      html += '<td>' + (gaps || '—') + '</td></tr>';
    });
    html += '</tbody></table></div>';
  }
