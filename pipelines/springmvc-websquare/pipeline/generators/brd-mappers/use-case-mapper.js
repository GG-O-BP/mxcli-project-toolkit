...
function buildFlow(screen, links) {
  // matchedBy vocabulary: java-angular values kept, springmvc-websquare linker values added
  // ('submission-endpoint-match', 'popup-opens') — in-place adaptation, not a fork.
  const apiLinks = links.filter(l => ['api-call-match', 'api-path-match-no-verb', 'submission-endpoint-match'].includes(l.matchedBy));
  const compositionLinks = links.filter(l => l.matchedBy === 'composes-component');
  const dialogOpensLinks = links.filter(l => l.matchedBy === 'dialog-opens' || l.matchedBy === 'popup-opens');

  // Read/write split. REST sources encode it in the HTTP verb (first token of via). The nbase
  // stack is RPC-over-POST — every submission is POST and intent lives in the URL's last
  // segment convention instead: '/get.' = read, '/reg.'|'/save.'|'/del.'|'/upd.' = write.
  const isWrite = (via) => {
    const verb = (via || '').split(' ').find(t => WRITE_METHODS.has(t) || t === 'GET');
    if (/\/get\.\w+$/.test(via || '')) return false;
    if (/\/(reg|save|del|upd|set|mod)\.\w+$/.test(via || '')) return true;
    return verb ? WRITE_METHODS.has(verb) : false;
  };
  const writeActions = apiLinks.filter(l => isWrite(l.via));
  const readActions   = apiLinks.filter(l => !isWrite(l.via));
...
function mapUseCases(screens) {
  return screens.map((s, i) => {
    ...
    return {
      ...
      screen:         s.name,
      uiPattern:      (s.widgetSummary || {}).hasListUI ? 'list'
                        : (s.widgetSummary || {}).hasFormUI ? 'form'
                        : s.screenKind === 'list' ? 'list'
                        : (s.screenKind === 'form' || s.screenKind === 'popup-form') ? 'form'
                        : 'detail',
      linkedLogics,
