...
function inferUiPattern(screen) {
  ...
  // websquare-extractor sets screenKind instead of hasListUI/hasFormUI flags —
  // in-place adaptation, per migration-pipeline.md's "patch mappers, don't fork" rule.
  if (screen.screenKind === 'list')  return (ws.formFields || []).length ? 'list+form' : 'list';
  if (screen.screenKind === 'form' || screen.screenKind === 'popup-form') return 'form';
  return 'detail';
}
