...
async function generate(kbDir, outDir, opts = {}) {
  ...
  for (const [modName, arts] of Object.entries(modules)) {
    ...
    try {
      ...
      // iBATIS <sql> fragments are shared SQL snippets, not callable logic — they must not
      // surface as implementation units (e.g. the paging clause is replaced by standard
      // Mendix datagrid paging, triage ③#9), so they are excluded from microflow mapping.
      const microflows     = mapMicroflows(arts.logics.filter(l => l.statementKind !== 'fragment'));
