...
class Merger {
  constructor({ extractedDir, knowledgeBaseDir, blueprintDir, logger }) {
  ...
  _loadAll() {
    const sources = ['springmvc','ibatis','websquare'];
    const items = [];
  ...
  _emit(items, crossRef) {
    const out = this.knowledgeBaseDir;
  ...
  _writeGapsReport(crossRef, items) {
    ...
    const reportsDir = path.join(this.knowledgeBaseDir, 'reports');
  ...
  _writeSummary(items, crossRef) {
    ...
    fs.writeFileSync(path.join(this.knowledgeBaseDir, 'summary.md'), lines.join('\n'), 'utf8');
  }
}
...
if (require.main === module) {
  ...
  // config.knowledgeBaseDir points at the per-project analysis folder — see migration-pipeline.md's
  // "Project Workspace Convention." Falls back to a local knowledge-base/ (gitignored scratch
  // space) only when no knowledgeBaseDir is configured, e.g. quick standalone testing.
  const kbDir = config.knowledgeBaseDir || path.join(__dirname, '..', 'knowledge-base');
  const merger = new Merger({
    extractedDir: path.join(kbDir, 'extracted'),
