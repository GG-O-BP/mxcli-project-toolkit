...
- [ ] 프롬프트 실행
  ...
  경로를 해석한다: 툴킷 경로, 소스 폴더, 분석 폴더, 대상 .mpr(이미 있으면 경로,
  없으면 "5단계 시작 시 mxcli로 생성"으로 표기하고 생성 시 갱신). 프로젝트 고유 사실 섹션은
...
- [ ] 프롬프트 실행
  ...
  ```
  먼저 읽기: 툴킷의 bug-logs/mxcli-bugs.md (프로젝트 생성·오픈 관련 알려진 이슈 확인)
  ...
  확인/생성된 .mpr 경로를 CLAUDE.md의 대상 .mpr 항목에 기록하고,
  Studio Pro로 열어 빈 상태에서 Run Locally가 성공하는지 확인한 뒤
